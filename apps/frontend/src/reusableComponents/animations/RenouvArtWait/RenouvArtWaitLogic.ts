import type { RenouvArtWaitProps } from "./RenouvArtWait";
...
export class RenouvArtLogic {
    ...
    drawBalls() {
        for (let i = 0; i < this.balls.length; i++) {
            this.ctx.fillStyle = `RGB(${(this.color.r + i) % 255},${(this.color.g + i) % 255},${(this.color.b + i) % 255})`;
            this.ctx.beginPath();
            this.ctx.arc(
